...
const VideoLayer = ({ id, clip, asset, updatePropertyValue, setSelectedClipId, selectedClipId, time, parentClip, projectId, clips }) => {
    ...
    useEffect(() => {
        const initVideo = async () => {
            ...
            const videoUrl = `${FILE_API_ENDPOINTS.DOWNLOAD}/${projectId}/${asset.url}`;
    ...
    useEffect(() => {
        if (videoTexture?.video) {
            videoTexture.video.currentTime = time;
        }
    }, [time, videoTexture]);
            ...
            <group position={[-anchorPoint.x, -anchorPoint.y, -anchorPoint.z]}>
                ...
                >
                    <mesh position={[width / 2, height / 2, 0]} scale={[1, -1, 1]} onClick={handleClick}>
                        ...
                        <meshBasicMaterial map={videoTexture?.texture} side={THREE.DoubleSide} opacity={opacity} transparent={opacity < 1} />
                    </mesh>
                </PivotControls>
            </group>
        </group>
