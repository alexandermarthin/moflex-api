...
-   **`utils.jsx`** - Common utilities, constants, JSON polyfill, logging, and helper functions
...
The script is configured via the `CONFIG` object at the top of `main-export.jsx`:

```jsx
var CONFIG = {
    outputPath: "/path/to/project.json",  // Where to save the exported JSON
    logPath: "/path/to/log.txt",          // Where to save log messages
    logLevel: 1                            // 0=off, 1=errors+milestones, 2=verbose
};
```
...
- **0** - Logging disabled
- **1** - Errors and key milestones only (recommended for production)
- **2** - Verbose debug logging (useful for troubleshooting)

Update these values in `main-export.jsx` before running the script.
...
Verify that the file paths in the `CONFIG` object in `main-export.jsx` are correct for your system.
...
-   **v2.1** - Code cleanup: configurable paths/logging, consolidated repetitive code, bug fixes, safety improvements
