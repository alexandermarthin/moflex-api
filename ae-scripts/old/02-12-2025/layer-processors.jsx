...
// Global variables for data collection (these should match the ones in main-export.jsx)
var assets = {};
var clips = {};
...
function processTrackMatte(layer, compId) {
    try {
        // Use the correct property names from the After Effects documentation
        var trackMatteType = layer.trackMatteType;
        var trackMatteLayer = layer.trackMatteLayer;

        logToFile("Processing track matte for layer '" + layer.name + "': trackMatteType=" + trackMatteType + ", trackMatteLayer=" + (trackMatteLayer ? trackMatteLayer.name : "null"));

        // TrackMatteType.NO_TRACK_MATTE = 0, TrackMatteType.ALPHA = 1, TrackMatteType.ALPHA_INVERTED = 2,
        // TrackMatteType.LUMA = 3, TrackMatteType.LUMA_INVERTED = 4
        if (trackMatteType === 0 || trackMatteType === TrackMatteType.NO_TRACK_MATTE || trackMatteType === TrackMatteType.NONE) {
            logToFile("No track matte detected for layer '" + layer.name + "'");
            return null; // No track matte
        }

        // Get the matte layer ID from the actual track matte layer
        var matteLayerId = null;
        if (trackMatteLayer) {
            // The matte layer is the actual layer being used as the matte
            matteLayerId = compId + "_" + (trackMatteLayer.index - 1);
            logToFile("Track matte layer found: '" + trackMatteLayer.name + "' with ID: " + matteLayerId);
        } else {
            // Fallback: determine the matte layer (usually the layer above)
            if (layer.index > 1) {
                matteLayerId = compId + "_" + (layer.index - 2);
                logToFile("Using fallback matte layer ID: " + matteLayerId);
            }
        }

        // Map track matte mode to string representation
        var modeString = "";
        var inverted = false;
        ...
        logToFile("Track matte found for layer '" + layer.name + "': mode=" + modeString + ", inverted=" + inverted + ", matteLayerId=" + matteLayerId);

        return {
            mode: modeString,
            inverted: inverted,
            matteLayerId: matteLayerId,
            trackMatteType: trackMatteType, // Keep the original numeric value for reference
            matteLayerName: trackMatteLayer ? trackMatteLayer.name : null,
        };
    } catch (trackMatteError) {
        logToFile("Error processing track matte for layer '" + layer.name + "': " + trackMatteError.toString());
        return {
