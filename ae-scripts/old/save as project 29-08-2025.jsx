...
try {
    ...
    // After Effects Project Export Script
    // Exports project data including:
    // - Assets (footage, compositions, folders)
    // - Clips (layers with properties, effects, keyframes)
    // - Masks (with paths, feathering, opacity, expansion)
    // - Text layers with formatting
    // - Shape layers with contents
    // - Transform properties (position, scale, rotation, etc.)
    // - Effects and their parameters
    // - Audio properties
    //
    // Improvements made:
    // - Added mask export functionality
    // - Simplified transform property processing
    // - Improved JSON handling with native support detection
    // - Better error handling consistency
    // - Cleaner property processing logic
            ...
            try {
                ...
                try {
                    ...
                    if (layer.mask && layer.mask.numMasks > 0) {
                        ...
                        logToFile("Processed " + layerData.masks.length + " masks for layer: " + layer.name);
                    }
                    ...
                    if (!masksFound) {
                        try {
                            ...
                            if (masksGroup && masksGroup.numProperties > 0) {
                                        ...
                                        if (maskGroup && maskGroup.name && maskGroup.name.indexOf("Mask") !== -1) {
                                            ...
                                            for (var debugIndex = 1; debugIndex <= maskGroup.numProperties; debugIndex++) {
                                                try {
                                                    ...
                                                    if (debugProp) {
                                                        logToFile(
                                                            "  Property " + debugIndex + ": '" + debugProp.name + "' (type: " + debugProp.propertyType + ", matchName: '" + debugProp.matchName + "')"
                                                        );
                                                    ...
                                                    if (maskProp) {
                                                        ...
                                                        if (maskProp.name === "Mask Path" || maskProp.name === "Path" || maskProp.name === "Shape" || maskProp.matchName === "ADBE Mask Shape") {
                                                            ...
                                                            try {
                                                                ...
                                                                logToFile(
                                                                    "Path value type: " + typeof pathValue + ", has vertices: " + (pathValue && pathValue.vertices ? pathValue.vertices.length : "no")
                                                                );
                                ...
                                if (layerData.masks.length > 0) {
                                    logToFile("Processed " + layerData.masks.length + " masks via property groups for layer: " + layer.name);
                                }
                            }
    ...
    function processPropertyGroup(propertyGroup, output, is3D) {
        try {
            ...
            if (propertyGroup.name === "Transform") {
                ...
                try {
                    var positionProp = propertyGroup.property("Position");
                ...
                // Process transform properties by index (like the backup version)
